...
public class MemberActiveBorrowsLatenessHandler implements MemberLatenessHandler {
    @Override
    public boolean wasLate(ResidentMember member, List<BorrowingDetails> borrowingDetails) {
        ...
        return borrowingDetails.stream().filter(BorrowingDetails::isActive)
                .anyMatch(val -> ChronoUnit.DAYS.between(val.getBorrowedAt(), now) > RESIDENT_LATENESS_IN_DAYS);
    }
